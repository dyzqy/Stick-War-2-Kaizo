package com.brockw.stickwar.engine.Ai
{
   ...
   public class ChromeAi extends UnitAi
   {
       
      
      public function ChromeAi(param1:ChromeElement)
      {
         super();
         unit = param1;
      }
      ...
      override public function update(param1:StickWar) : void
      {
         ...
         if(currentCommand.type == UnitCommand.SPLIT)
         {
            ChromeElement(unit).split();
         }
         else if(currentCommand.type == UnitCommand.CONVERT)
         {
            _loc2_ = int(ConvertCommand(currentCommand).targetId);
            if(_loc2_ in param1.units)
            {
               ...
               else
               {
                  ...
                  if(_loc3_ is Unit && Unit(_loc3_).team != unit.team)
                  {
                     ChromeElement(unit).convertSpell(Unit(_loc3_));
                     nextMove(param1);
                  }
                  else
                  {
         ...
         else if(currentCommand.type == UnitCommand.TELEPORT)
         {
            ...
            {
               ChromeElement(unit).teleportSpell(TeleportCommand(currentCommand).realX,TeleportCommand(currentCommand).realY);
               nextMove(param1);
            }
         }
         else
